...
for codigo, dados in quartos.items():
    ...
    preco = dados["preco"]
    disponivel = "OCUPADO" if not dados["disponivel"] else "VAGO"
...
try:
    dias = int(input("Quantos dias ficará hospedado(a): ").strip())
except ValueError:
    logging.error("Número inválido, digite apenas digitos.")
    sys.exit(1)
...
print(f"{nome} você escolheu o quarto {num_quarto}, e vai custar: R${total:.2f}")
