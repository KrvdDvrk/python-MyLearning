...
class KivySum(App):
    def build(self):
        ...
        # label
        self.window.add_widget(Label(text="Calculator", font_size=40, color="#ffcc00"))

        # inputs
        self.x = TextInput(multiline=False, padding_y=(2, 2), size_hint=(1, 0.5), font_size=30)
        self.window.add_widget(self.x)
        self.y = TextInput(multiline=False, padding_y=(2, 2), size_hint=(1, 0.5), font_size=30)
        ...
        # Buttons
        calcula = Button(text="Calcula", size_hint=(1, 0.5), bold=True, background_color="#00FFCE")
        calcula.bind(on_press=self.calcula)
        self.window.add_widget(calcula)

        sair = Button(text="Sair", size_hint=(0.5, 0.5))
